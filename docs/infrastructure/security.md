...
- **graphrag's own log files are the exceptions, and they are graphrag's, not Groundly's.** (a) `<subject>/graph/logs/indexing-engine.log`, written on every graph build (`--debug` raises it from INFO to DEBUG, which includes extraction prompts, i.e. chunk text); excluded from bundle export alongside `graph/cache/`. (b) `./logs/query.log` — the query-time `GraphRagConfig` sets no root, so graphrag logs into the **current working directory** on every graph-arm retrieve, wherever the command happened to run. Known and open (2026-08-03): it matters most under `groundly mcp`, whose CWD belongs to the host app, not to Groundly. Content audited on a real 48-question run: LLM error tracebacks only — no query text, no chunk text — so it is a tidiness and containment defect, not a disclosure one, but it is outside `~/.groundly/` and must be rerooted to `<subject>/graph/logs/` to match the build path.
