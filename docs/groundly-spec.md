...
17. **Standalone image ingestion** (Paul, 2026-07-20): raster images (`.png .jpg .jpeg .tif .tiff .bmp .webp`) index as first-class inputs. docling routes `InputFormat.IMAGE` through the same `StandardPdfPipeline` as PDFs, so an image is treated as a one-page scanned PDF — OCR'd on the identical pinned `RapidOcrOptions(backend="onnxruntime")` path (decision 14), single-page (page-1) attribution, `--ocr-lang` applies unchanged (decision 15). `_extract_docling` registers the IMAGE format with the same `PdfPipelineOptions` used for PDF; **this registration is load-bearing** — without it docling's IMAGE default falls back to EasyOCR (runtime model downloads), breaking the local/offline pin. OCR-only: no VLM captioning/description (that would require a provider, breaking zero-key indexing). Consequences: partial reversal of descope #5 — printed/typeset "photo notes" now index (handwriting and vision fallback remain out); a text-free image gets the same "no readable text — OCR found nothing to extract" message; **no interchange change** — manifest and `format_version` unchanged, embeddings/graph identical, so no re-index migration; citation accuracy on image sources is bounded by OCR quality. Audio and XML docling formats remain out of scope (add when needed).
