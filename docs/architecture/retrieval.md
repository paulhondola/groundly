...
**Arm 3 was the production arm until decision 28 and is not any more.** It was measured, it lost, and the honest configuration on this corpus is vector-only. The arms stay selectable in `groundly eval` — retiring an arm from the product path is not the same as deleting it, and a negative result that cannot be re-run from shipped code is not a result. `ARMS` is what the eval may score; `agents.ask.PRODUCT_ARMS` is what a user question may reach, and the two being different is the whole content of the decision.
