...
- LLM clients are constructed **only** in `unilearn/llm/` — OpenAI-compatible `base_url` + model + key, **per call class** (`chat`, `generation`, `extraction`, `router`) from `~/.unilearn/config.toml`. Never hardcode a provider; cloud keys and LM Studio/Ollama are the same code path.
...
- `unilearn serve` binds **127.0.0.1 only**.
