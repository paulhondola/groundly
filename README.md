...
```
$ unilearn index <SUBJECT> ./slides/
  ✓ 12 PDFs → 384 chunks → embedded (bge-m3, local) → indexed
$ # wire into Claude Code:  { "command": "unilearn", "args": ["mcp"] }
```
...
3. **A portable index** — `unilearn export <SUBJECT>` → one file; a coursemate imports it and their agent uses it directly. The expensive artifacts (knowledge graph, verified decks) are built once and shared.
...
- **Grounded Q&A** — hybrid retrieval (bge-m3 dense + learned sparse + BM25, reranked; optional GraphRAG for synthesis questions), via MCP tools or `unilearn ask`
- **Verified quizzes & coding challenges** — generated by UniLearn (your API key) *or by your agent* (no key needed) — either way, nothing unverified is stored
